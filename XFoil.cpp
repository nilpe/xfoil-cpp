...
void XFoil::writeString(
    std::string str, bool bFullReport)
{
...
void XFoil::hipnt(double chpnt, double thpnt)
{
    ...
    if (fabs(arot) > 1.0)
    {
        ...
        oss << std::fixed << std::setprecision(2) << arot;
        strong = "Current chordline angle: " + oss.str() + "\nproceeding anyway...";
...
bool XFoil::iblpan()
{
    ...
    if (iblmax > IVX)
    {
        ...
        std::ostringstream oss;
        oss << iblmax;
        str = "Increase IVX to at least " + oss.str() + "\n";
...
bool XFoil::mrchdu()
{
    ...
    {//2000
        ...
        {//1000
            ...
            oss1 << ibl;
            oss2 << is;
            oss3 << std::fixed << std::setprecision(3) << dmax;
            str = "     mrchdu: convergence failed at " + oss1.str() + " ,  side " + oss2.str() + ", res =" + oss3.str() + "\n";
...
bool XFoil::mrchue()
{
    ...
    {//2000
        ...
        {// 1000
            ...
            for (itbl = 1; itbl <= 25;itbl++) {//100
                ...
                if (direct)
                {
                    ...
                    else
                    {
                        ...
                        oss1 << ibl;
                        oss2 << std::fixed << std::setprecision(3) << htarg;
                        str = "     mrchue: inverse mode at " + oss1.str() + "    hk =" + oss2.str() + "\n";
            ...
            oss2 << is;
            oss3 << std::fixed << std::setprecision(3) << dmax;
            str = "     mrchue: convergence failed at " + oss1.str() + ",  side " + oss2.str() + ", res = " + oss3.str() + "\n";
...
bool XFoil::mrcl(double cls, double& m_cls, double& r_cls)
{
    ...
    if (rrat > 100.0)
    {
        ...
        writeString(str, true);
        std::ostringstream oss;
        oss << std::fixed << std::setprecision(0) << reinf1 * 100.0;
        str = "      artificially limiting Re to " + oss.str() + "\n";
    ...
    for (is = 1;is <= 2;is++)
    {
        ...
        for (ibl = 2;ibl <= nbl[is];ibl++)
        {
            ...
            if (ibl == itran[is] && !tran)
            {
                ...
                oss1 << ampl1;
                oss2 << ampl2;
                std::string str = "setbl: xtr???  n1=" + oss1.str() + " n2=" + oss2.str() + ": \n";
        ...
        if (tforce[is])
        {
            std::ostringstream oss1, oss2, oss3;
            oss1 << is;
            oss2 << std::fixed << std::setprecision(4) << xoctr[is];
            oss3 << itran[is];
            std::string strOut = "     Side " + oss1.str() + ", forced transition at x/c = " + oss2.str() + " " + oss3.str() + "\n";
        ...
        else
        {
            ...
            oss2 << std::fixed << std::setprecision(4) << xoctr[is];
            oss3 << itran[is];
            std::string strOut = "     Side " + oss1.str() + ",  free  transition at x/c = " + oss2.str() + " " + oss3.str() + "\n";
            //TRACE(strOut);
            writeString(strOut);
        }
...
bool XFoil::xifset(int is)
{
    ...
    if (xiforc < 0.0) {
        ...
        std::ostringstream oss;
        oss << is;
        std::string str = " ***  stagnation point is past trip on side " + oss.str() + "\n";
    ...
    double xcir[], double ycir[], double scir[], double qcir[])
{
    ...
    *ncir = nc;
    ...
    std::ostringstream oss;
    oss << std::fixed << std::setprecision(4) << dalfa;
    std::string str = "qccalc: cl convergence failed.  dalpha =" + oss.str();
...
void XFoil::HanningFilter(double cfilt, std::stringstream& ts)
{
    ...
    std::ostringstream oss1, oss2, oss3;
    oss1 << std::fixed << std::setprecision(4) << (algam / dtor);
    oss2 << std::fixed << std::setprecision(6) << clgam;
    oss3 << std::fixed << std::setprecision(6) << cmgam;
    ...
    for (int kqsp = 1; kqsp <= nqsp; kqsp++)
    {
        ...
        std::ostringstream oss1, oss2, oss3;
        oss1 << std::fixed << std::setprecision(4) << (alqsp[kqsp] / dtor);
        oss2 << std::fixed << std::setprecision(6) << clqsp[kqsp];
        oss3 << std::fixed << std::setprecision(6) << cmqsp[kqsp];
